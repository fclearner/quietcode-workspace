...
const curated = {
  ...
  ...require('./monotonic-stack-mastery-track'),
  'two-sum': {
    id: 1,
  ...
  'design-hashset': {
    id: 705,
    summary: '不使用语言内置的 Set、set 或 unordered_set，设计一个整数集合，支持 add(key)、remove(key) 和 contains(key)。add 插入键，remove 删除键，contains 判断键是否存在；重复插入和删除不存在的键都应安全处理。',
    input: '每行是一条操作：add key、remove key 或 contains key。\nkey 是 0 到 1,000,000 之间的整数，至少包含一条操作。',
    output: '每个 contains 操作输出一行：存在时输出 true，否则输出 false。add 和 remove 不输出。',
    examples: [
      { input: 'add 1\nadd 2\ncontains 1\ncontains 3\nadd 2\ncontains 2\nremove 2\ncontains 2', output: 'true\nfalse\ntrue\nfalse' },
      { input: 'contains 0\nadd 0\ncontains 0\nremove 0\ncontains 0', output: 'false\ntrue\nfalse' },
      { input: 'add 1000000\nadd 42\nremove 7\ncontains 1000000\nremove 1000000\ncontains 1000000\ncontains 42', output: 'true\nfalse\ntrue' }
    ],
    templates: {
      javascript: "const lines = require('fs').readFileSync(0, 'utf8').trim().split(/\\n/);\n\nclass MyHashSet {\n  constructor() {\n    // TODO: 初始化你的数据结构\n  }\n  add(key) {\n    // TODO: 插入 key\n  }\n  remove(key) {\n    // TODO: 删除 key\n  }\n  contains(key) {\n    // TODO: 判断 key 是否存在\n    return false;\n  }\n}\n\nconst set = new MyHashSet();\nconst output = [];\nfor (const line of lines) {\n  const [operation, keyText] = line.trim().split(/\\s+/);\n  const key = Number(keyText);\n  if (operation === 'contains') output.push(set.contains(key) ? 'true' : 'false');\n  else set[operation](key);\n}\nconsole.log(output.join('\\n'));\n",
      python: "import sys\nlines = sys.stdin.read().strip().splitlines()\n\nclass MyHashSet:\n    def __init__(self):\n        # TODO: 初始化你的数据结构\n        pass\n    def add(self, key):\n        # TODO: 插入 key\n        pass\n    def remove(self, key):\n        # TODO: 删除 key\n        pass\n    def contains(self, key):\n        # TODO: 判断 key 是否存在\n        return False\n\nvalues = MyHashSet()\noutput = []\nfor line in lines:\n    operation, key_text = line.split()\n    key = int(key_text)\n    if operation == 'contains':\n        output.append('true' if values.contains(key) else 'false')\n    else:\n        getattr(values, operation)(key)\nprint('\\n'.join(output))\n",
      cpp: "#include <iostream>\n#include <string>\n#include <vector>\nusing namespace std;\n\nclass MyHashSet {\npublic:\n    MyHashSet() {\n        // TODO: 初始化你的数据结构\n    }\n    void add(int key) {\n        // TODO: 插入 key\n        (void)key;\n    }\n    void remove(int key) {\n        // TODO: 删除 key\n        (void)key;\n    }\n    bool contains(int key) {\n        // TODO: 判断 key 是否存在\n        (void)key;\n        return false;\n    }\n};\n\nint main() {\n    MyHashSet values; string operation; int key;\n    while (cin >> operation >> key) {\n        if (operation == \"contains\") cout << (values.contains(key) ? \"true\" : \"false\") << '\\n';\n        else if (operation == \"add\") values.add(key);\n        else values.remove(key);\n    }\n}\n"
    },
    solutions: {
      javascript: "class MyHashSet {\n  constructor() { this.present = new Uint8Array(1_000_001); }\n  add(key) { this.present[key] = 1; }\n  remove(key) { this.present[key] = 0; }\n  contains(key) { return this.present[key] === 1; }\n}\n",
      python: "class MyHashSet:\n    def __init__(self):\n        self.present = bytearray(1_000_001)\n    def add(self, key):\n        self.present[key] = 1\n    def remove(self, key):\n        self.present[key] = 0\n    def contains(self, key):\n        return self.present[key] == 1\n",
      cpp: "class MyHashSet {\n    vector<bool> present;\npublic:\n    MyHashSet() : present(1000001, false) {}\n    void add(int key) { present[key] = true; }\n    void remove(int key) { present[key] = false; }\n    bool contains(int key) { return present[key]; }\n};\n"
    }
  }
};
